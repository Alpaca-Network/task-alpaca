...
task('e2e', 'Runs all e2e tests')
  ...
  .setAction(async (taskArgs: any, hre: HardhatRuntimeEnvironment) => {
    ...
    try {
      ...
      testResults['Anthropic claude-3-5-sonnet-20240620'] = truncateMessage(
        e.message
      );
    }
    ...
    const transformedResults = Object.entries(testResults).map(
      ([testName, result]) => ({
        Test: testName,
        Result: result,
      })
...
function truncateMessage(message: string) {
  return message.length > MAX_ERROR_LEN
    ? message.slice(0, MAX_ERROR_LEN - 3) + '...'
    : message;
}
