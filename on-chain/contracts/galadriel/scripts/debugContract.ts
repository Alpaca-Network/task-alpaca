...
async function getMessages(
  ...
  if (!messages.length) {
    try {
      const newMessages = await oracle.getMessagesAndRoles(
        promptId,
        promptCallbackId
      );
...
async function addOracleResponse(
  ...
  try {
    await oracle
      ...
      .addResponse(promptId, promptCallbackId, response, '');
  } catch (e) {
    error = e;
  }
  if (error) {
...
describe('Contract', function () {
  ...
  describe('Basic usage', function () {
    ...
    it('Oracle can add response', async () => {
      ...
      expect(messages.length).to.above(
        1,
        'Expected message history to contain at least 2 messages'
      );
  ...
  describe('Error handling', function () {
    ...
    it('Oracle cannot add 2 responses', async () => {
      ...
      expect(isErrored).to.equal(
        true,
        'Oracle should not be able to send 2 responses in a row'
      );
    });
  });
});
