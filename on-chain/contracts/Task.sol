...
contract Task {
  ...
  function evaluateAgent(address agent) public view returns (uint256) {
    string memory agentOutput = Agent(agent).sendMessage(
      string.concat('Complete the following task: ', description)
    );

    string memory evaluationOutput = Agent(criticLLMAddress).sendMessage(
      string.concat(
