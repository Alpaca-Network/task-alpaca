...
contract TaskRegistry {
    ...
    function createTask(string memory _description, address _agent, address _evaluator) public payable {
        require(msg.value > 0, "Reward must be greater than 0");
        Task memory newTask = Task({description: _description, agent: _agent, evaluator: _evaluator, reward: msg.value});
        tasks.push(newTask);
        emit TaskCreated(tasks.length - 1, _description, msg.value, _agent, _evaluator);
    }
