...
async function main() {
  ...
  const incentiveAmount: number = 0.01;

  await deployTask("this is a task description", 100, 1000, incentiveAmount);
}

async function deployTask(description: string, pricePerExecution: number, IncentiveDurationInBlocks: number, incentiveAmount: number) {
    const task = await ethers.deployContract("Task", [description, pricePerExecution, IncentiveDurationInBlocks], {
        value: ethers.utils.parseEther(incentiveAmount) // Convert Ether to wei
    });
